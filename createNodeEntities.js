...
function buildEntity({ name, data, schemas, createNodeId }) {
  ...
  const dataWithoutChildEntities = getDataWithoutChildEntities(
    data,
    childNodeKeys
  )
...
function createNodeEntities({ name, data, createNodeId, schemas }) {
  if (isArray(data)) {
    const entitiesArray = data.map(d =>
      buildEntity({
        name,
